...
const UserItem = React.memo(({ user, onlineUsers, onClick, onDelete }) => {
  const { user: currentUser } = useAuth();
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const isOnline = onlineUsers.has(user._id);
  const isAdmin = currentUser?.role === 'admin';

  // Optimize avatar: use Cloudinary transformations for smaller, faster images
  const avatarUrl = user.avatar?.includes('cloudinary')
    ? `${user.avatar}?w=48&h=48&c=fill&q=auto&f=auto`
    : user.avatar;
          ...
          {!showDeleteConfirm ? (
            ...
            <AnimatePresence>
              ...
              >
                ...
                <button onClick={handleDelete} className="px-2 py-1 text-xs bg-red-500 text-white rounded">Yes</button>
